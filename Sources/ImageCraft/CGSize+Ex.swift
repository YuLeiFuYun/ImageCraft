...
extension ImageCraftWrapper where Base == CGSize {
    var aspectRatio: CGFloat {
        base.height == 0.0 ? 1.0 : base.width / base.height
    }
    
    // resize 方法接受一个 desiredSize 和 contentMode 参数，返回调整后的 CGSize。
    func resize(to desiredSize: CGSize, for contentMode: ImageCraft.ContentMode = .scaleAspectFit) -> CGSize {
...
extension ImageCraftWrapper where Base == CGRect {
    // 根据给定的缩放比例，返回缩放后的 CGRect。
