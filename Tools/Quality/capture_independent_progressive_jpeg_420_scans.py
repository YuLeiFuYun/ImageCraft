...
DEFAULT_PROFILE = ROOT / "Evidence/Experiments/IndependentProgressiveJPEG420Scans/v1/profile.json"
DEFAULT_OUTPUT = ROOT / ".artifacts/program/T101/independent-progressive-jpeg-420-scans-v1.json"
...
def validate_scan_pair(
    *,
    ...
    if imagecraft != libjpeg:
        ...
        pixel = mismatch[0] // 3
        raise CaptureError(
            f"per-scan RGB differs from no-smoothing libjpeg: {case_id}/scan-{scan_number} "
    ...
    return {
        ...
        "exactNoSmoothingLibjpegRGB": True,
    }
...
def validate_case(
    *,
    ...
    completed = run(
        [
            str(imagecraft),
            "--independent-progressive-jpeg-420-scans",
            str(jpeg_path),
            "--output-directory",
            str(imagecraft_dir),
        ]
    ...
    if (
        ...
        != "imagecraft-independent-progressive-jpeg-420-scans-v1"
    ...
    if probe_completed.stderr.strip():
        raise CaptureError(
            f"libjpeg no-smoothing scan probe emitted diagnostics for {case_id}: "
            f"{probe_completed.stderr.strip()}"
        )
    probe_report = parse_json_stdout(probe_completed, f"libjpeg no-smoothing scans {case_id}")
    if (
        probe_report.get("decodeMode") != "no-smoothing"
        ...
        raise CaptureError(f"libjpeg no-smoothing scan report drifted: {case_id}")
    ...
    return {
        ...
        "allScansExactNoSmoothingLibjpegRGB": True,
    }
...
def main() -> int:
    ...
    if profile.get("profileID") != "IMAGECRAFT-INDEPENDENT-PROGRESSIVE-JPEG-420-SCANS-V1":
    ...
    with tempfile.TemporaryDirectory(prefix="imagecraft-progressive-420-scans-") as temp_raw:
        ...
        report = {
            "schemaVersion": 1,
            "evidenceVersion": "imagecraft-independent-progressive-jpeg-420-scans-conformance-v1",
            "status": "source-bound-package-backend-scan-conformance",
            ...
            "summary": {
                ...
                "allScansExactNoSmoothingLibjpegRGB": all(
                    case["allScansExactNoSmoothingLibjpegRGB"] for case in results
                ),
        ...
        print(
            "independent progressive JPEG 4:2:0 per-scan conformance captured: "
