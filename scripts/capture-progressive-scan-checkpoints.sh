...
cd "$ROOT"
OUTPUT=${1:-.artifacts/performance/progressive-scan-checkpoints.json}
RAW_OUTPUT=${2:-}
...
import json, sys
for variant in json.load(open(sys.argv[1]))['variants']:
    print(variant['id'])
PY
while IFS= read -r variant; do
    "$EVIDENCE_BIN" --progressive-scan-checkpoints "$MANIFEST" "$variant" \
        > "$TMPDIR_ROOT/$variant-a.json"
    "$EVIDENCE_BIN" --progressive-scan-checkpoints "$MANIFEST" "$variant" \
        > "$TMPDIR_ROOT/$variant-b.json"
done < "$TMPDIR_ROOT/variants.txt"
mkdir -p "$(dirname "$OUTPUT")"
python3 Tools/Performance/aggregate_progressive_scan_checkpoints.py \
    --manifest "$MANIFEST" \
