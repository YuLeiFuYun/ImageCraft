...
test -x "$EXECUTABLE"
for case_id in \
    progressive-jpeg-timeline-fit-512-chunk-1024 \
    progressive-jpeg-timeline-fit-512-chunk-32768
do
    repetition=1
    while [ "$repetition" -le "$PROCESS_REPETITIONS" ]; do
        "$EXECUTABLE" \
            --progressive-timeline-case "$case_id" \
...
Tools/Performance/aggregate_progressive_timeline.py \
    --case-directory "$CASE_DIRECTORY" \
    --swift-version "$SWIFT_VERSION" \
    --expected-process-repetitions "$PROCESS_REPETITIONS" \
